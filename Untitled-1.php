...
<html lang="en">
    ...
    <body>
        ...
        <input type = "date" id = "datepicker">
            ...
            ><ion-icon name="arrow-dropright-circle"></ion-icon></input>
             
        </div>
        
        <div style="height:370px; overflow-y: scroll;">
        <script> 
        let day = <?php echo $_GET["day"]; ?>
        let month = <?php echo $_GET["month"]; ?>
        let year = <?php echo $_GET["year"]; ?>
        $("document").ready(() => {
            $("#prev_day").attr("value") = newday;
            // find some way to fetch the current value of day, month, year
            // 
        });
        alert
        ...
        elem.value = inputValue;
            ...
            /*
            form (prev button) action="" method="get"
            - hidden type, name=prevday, value use js dom (current day - 1)
            - hidden type, name=prevmonth, value use js dom (current month)
            - hidden type, name=year, value use js dom (current year)
            - submit button
            form (next button) action="" method="get"
            - hidden type, id andname=day, value use js dom (current day + 1)
            - hidden type, id and name=month, value use js dom (current month)
            - hidden type, name=year, value use js dom (current year)
            - submit button

            when preload the page with php
            do calc on day first

            minus
            for exmaple if regular day, then -1 will do
            if day=1, then month -1
            if day=1 and month=1, year-1 and set month to 12, day to 31

            plus
            if day=28 and month=2 and year % 4 !=0, month +1 and day=1
            if day=31 confirm month +1 and day=1
            if day=30 and month=""  
            */
        </script>
        <?php 
